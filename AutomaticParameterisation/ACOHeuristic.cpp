...
/*
 * Creates and configures ACO object using the created variables.
 * Runs optimise function to find a local optimum.
 * Adds local optimum as best solution.
 *
 * ACO updates pheromones based on length of customer route rather than total route of customers,
 * charging stations, and depots.
 */
void ACOHeuristic(int numAnts,int iterations, int probabilityArraySize,int twoOptIteration, int randomSearchIteration,
                  double pheromoneDecrease, double Q,double alpha, double beta){
...
//
//void MMACOHeuristic(){
//    int numAnts=8, iterations = 100, probabilityArraySize = 2, twoOptIteration = 3,randomSearchIteration = 3;
//    double pheromoneDecrease = 0.8, Q = 80,alpha = 0.8, beta=0.8, pBest = 0.005;
